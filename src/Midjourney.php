...
namespace Stratonit\Midjourney;

use App\Core\DiscordDataProcessor;
use Exception;
...
class Midjourney {
    ...
    public function getMessages(string $prompt)
    {
        $response = self::$client->get('channels/' . self::$channel_id . '/messages');
        $response = json_decode((string) $response->getBody());
        $processor = new DiscordDataProcessor($response,$prompt);
        $processor->process();
        return $response;
...
//                str_starts_with($item->content, "**{$prompt} -- - <@" . self::$user_id . '>') and
                str_starts_with($item->content, "**{$prompt}") and
